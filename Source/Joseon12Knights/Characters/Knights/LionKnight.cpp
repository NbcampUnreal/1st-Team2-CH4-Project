...
void ALionKnight::Skill(const FInputActionValue& Value)
{
	ServerSkill();
}

void ALionKnight::ServerSkill_Implementation()
{
	MulticastSkill();
}

void ALionKnight::MulticastSkill_Implementation()
{
	if (!bCanUseSkill)
	{
		UE_LOG(LogTemp, Warning, TEXT("LionKnight W is	CoolDown.."));
		return;
	}
...
// ====================
void ALionKnight::Ultimate(const FInputActionValue& Value)
{
	ServerUltimate();
}

void ALionKnight::ServerUltimate_Implementation()
{
	MulticastUltimate();
}

void ALionKnight::MulticastUltimate_Implementation()
{
	if (!bCanUseUltimate)
	{
...
//  디버그 함수
// ==========
void ALionKnight::DebugPrintSelfBuffs() const
{
	if (UBuffComponent* BuffComp = const_cast<ALionKnight*>(this)->FindComponentByClass<UBuffComponent>())
	{
